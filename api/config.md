...
### `test_flush`
The flag whether [`Test Workflow`](/api/test.md) should flush and drop the previous test collections or simply continue with them. It's not supposed to be set by a package, rather by the CLI. Default `False`.
